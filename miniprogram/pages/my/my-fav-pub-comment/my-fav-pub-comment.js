...
Page({
  ...
  onLoad: function (options) {
    ...
    if (typeid == myFav) {
      ...
      db.getFavCommentListViaUser().then(result => {
        wx.hideLoading()
        ...
        db.getCommentListViaIdList(reviewIdList).then(reviewResult => {
      ...
      db.getSubmittedCommentList().then(result => {
        wx.hideLoading()
  ...
  setFilmInfo(reviewData) {
    ...
    db.getFilmListById(filmIdList).then(filmResult => {
  ...
  redirectToReviewDetails: function (eve) {
    ...
    wx.navigateTo({
      url: '../my-comment/my-comment?id=' + id,
    })
